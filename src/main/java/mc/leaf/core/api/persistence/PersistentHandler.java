...
import mc.leaf.core.api.persistence.wrappers.LocationKeyWrapper;
...
public class PersistentHandler<K extends ILeafModule, T extends Persistable> implements PersistentDataType<PersistentDataContainer, T> {
    ...
    public PersistentDataContainer toPrimitive(@NotNull T complex, @NotNull PersistentDataAdapterContext context) {
        ...
        try {
            ...
            for (Field field : fields) {
                ...
                } else if (data instanceof Enum<?> enumeration) {
                    container.set(this.getKey(persist.key()), STRING, enumeration.name());
                } else {
    ...
    public T fromPrimitive(@NotNull PersistentDataContainer primitive, @NotNull PersistentDataAdapterContext context) {
        ...
        try {
            ...
            for (Field field : fields) {
                ...
                // Can't really do much about it, it's Java Reflection
                ...
                if (field.getType() == String.class) {
                    ...
                    String key = primitive.get(this.getKey(persist.key()), STRING);

                    for (Enum<?> possibleValue : ((Class<Enum<?>>) field.getType()).getEnumConstants()) {
                        if (possibleValue.name().equals(key)) {
                            field.set(instance, possibleValue);
                        }
                    }
                } else {
                    this.getModule().getPlugin().getLogger()
